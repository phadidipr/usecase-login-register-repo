...
•	A form takes in an email address, first name, last name, password, and profile image; there is also an option to include an employee id within the form\
	-	Before submitting, the form checks if there is an employeeId value, and if there is one, it checks if it is a valid employee id with 12 alphanumeric characters combined with a Presidio email address to match. If both of these checks pass, registration moves to the next step; otherwise, it stops the process and displays an alert showing what needs correction\
